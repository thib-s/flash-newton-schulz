...
def update(
    ...
    update_ = momentum * momentum_buffer + (1 - momentum) * grad

    if step is not None:
        dir_name = f"grads/{cmd_args.ns_impl}/"
        os.makedirs(dir_name, exist_ok=True)
        name_weights = os.path.join(
            dir_name, f"update_{grad.size(0)}_{grad.size(1)}_{step}.pth"
        )
        torch.save(update_, name_weights)
...
class Hyperparameters:
    ...
    train_files = os.path.join(
        os.environ.get("FINEWEB"), "fineweb_train_*.bin"
    )  # input .bin to train on
    val_files = os.path.join(
        os.environ.get("FINEWEB"), "fineweb_val_*.bin"
    )  # input .bin to train on
    ...
    val_loss_every = (
        cmd_args.save_every  # every how many steps to evaluate val loss? 0 for only at the end
    )
...
for step in range(train_steps + 1):
    ...
    for opt in inner_optimizers:
        ...
        if isinstance(opt, Muon) and step % cmd_args.save_every == 0:
            opt.step(step=step)

            assert os.path.exists(f"grads/{cmd_args.ns_impl}")
            if os.path.exists(f"grads/{cmd_args.ns_impl}/loss_vals.csv") is False:
                df = pd.DataFrame(columns=["step", "val_loss"])
                df.to_csv(f"grads/{cmd_args.ns_impl}/loss_vals.csv", index=False)
            else:
                df = pd.read_csv(f"grads/{cmd_args.ns_impl}/loss_vals.csv")
                val_loss = float(val_loss.cpu())
                df = pd.concat(
                    [df, pd.DataFrame([{"step": step, "val_loss": val_loss}])],
                    ignore_index=True,
                )
                df.to_csv(f"grads/{cmd_args.ns_impl}/loss_vals.csv", index=False)
        else:
