...
dynamo.config.recompile_limit = 1000
...
def ns_AOLxMuon+(G: Tensor, iter=4, epsilon: float = 1e-7, dtype=torch.bfloat16):
    """
...
class Muon(torch.optim.Optimizer):
    ...
    Note: A later PR replaced Newton-Shulz with ns_AOLxMuon_plus for the orthogonalization step
    ...
    def __init__(
        self, params, lr=0.02, weight_decay=0.01, momentum=0.95, custom_sizing=True
    ):
    ...
    def step(self):
        ...
        for group, info in zip(self.param_groups, group_infos):
            ...
            # Reshape attn params from [hdim, dim*4] to [4,hdim,dim] to apply orthogonalization independently to Q,K,V,O
            param_idx = start_idx if start_idx < len(params) else 0
            if getattr(params[param_idx], "label", None) == "attn":
                for p in params[param_idx : param_idx + chunk_size]:
                    assert (
                        getattr(params[param_idx], "label", None) == "attn"
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, head_dim: int, num_heads: int):
        ...
        with torch.no_grad():
            self.qkvo_w.view(4, self.hdim, self.dim)[:3].uniform_(
                -bound, bound
            )  # init QKV weights
            self.qkvo_w.view(4, self.hdim, self.dim)[
                3
            ].zero_()  # init output weights to zero
    ...
    def forward(self, x: Tensor, attn_args: AttnArgs):
        ...
        q, k, v = (
            F.linear(
                x,
                self.qkvo_w.view(4, self.hdim, self.dim)[:3]
                .flatten(end_dim=1)
                .type_as(x),
            )
...
def distributed_data_generator(
    ...
    assert (
        num_tokens % (world_size * grad_accum_steps) == 0
    ...
    while True:
        ...
        if new_params is not None:
            ...
            assert (
                new_num_tokens % (world_size * grad_accum_steps) == 0
...
parser = argparse.ArgumentParser(
    description="NanoGPT Training with Flash Newton-Schulz Optimizer"
)
parser.add_argument(
    "--ns-iter", type=int, default=4, help="Number of Newton-Schulz iterations"
)
parser.add_argument(
    "--ns-impl",
    type=str,
    default="muon",
    choices=["muon", "muon+", "aol_muon_plus"],
...
ns_impls = {
    "muon": NS_muon,
    "muon+": NS_muon_plus,
    "aol_muon_plus": ns_AOLxMuon_plus,
}
...
class Hyperparameters:
    ...
    num_scheduled_iterations: int = (
        cmd_args.num_scheduled_iterations  # number of steps to complete lr and ws schedule
    )
    ...
    val_loss_every: int = (
        5  # every how many steps to evaluate val loss? 0 for only at the end
    )
...
# begin logging
logfile = None
global_logfile = "logs/global.txt"
if master_process:
    run_id = args.run_id
...
print0(code)
print0("=" * 100)
print0(f"NS iters: {cmd_args.ns_iter}")
print0("=" * 100)
...
optimizer2 = Muon(
    hidden_matrix_params + gate_params, lr=0.06, momentum=0.95, weight_decay=0.0
)
